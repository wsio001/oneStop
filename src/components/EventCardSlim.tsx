...
export default function EventCardSlim({ event }: EventCardSlimProps) {
  return (
    <div className="border-[0.5px] border-gray-400 rounded-lg mb-3 px-3 py-2">
      ...
      {/* Event Name */}
      <div className="text-md font-semibold text-gray-700 mb-1.5">
        {event.event_name}
      </div>

      {/* Location */}
      {event.location && (
        <div className="text-[12px] text-gray-800 mb-1 flex items-center gap-1">
          <span>📍</span>
      ...
      {(event.in_charge_raw || event.helpers_raw) && (
        <div className="text-[12px] text-gray-800 mb-1 flex items-start gap-1">
          <span>👤</span>
          <span>
            {event.in_charge_raw && (
      ...
      {/* Notes */}
      {event.notes && (
        <div className="text-[12px] text-gray-800 mt-2 pt-2 border-t border-gray-400 italic">
          {event.notes}
        </div>
      )}
    </div>
